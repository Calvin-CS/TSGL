...
/*!
 * \brief Draws a neat pattern of points to a RasterCanvas using OMP and takes in a command line
 ...
 * - The outer for loop is set up in a block pattern, and the inner for loop runs from 0 to the RasterCanvas width.
 *   - If the point's coordinate is even:
 *     - Draw the point normally.
 *   .
 *   - The function breaks from the outer for loop if the RasterCanvas is closed.
 *   .
 * - Sleep the internal timer of the RasterCanvas until the next draw cycle.
 * .
 * \param can Reference to the RasterCanvas being drawn to.
 * \param numberOfThreads Reference to the number of threads to use.
 */
void colorPointsFunction(RasterCanvas& can, int numberOfThreads) {
#pragma omp parallel num_threads(numberOfThreads)
  {
    int nthreads = omp_get_num_threads();  //Actual number of threads to use
    int id = omp_get_thread_num();
    int myPart = can.getWindowWidth() / nthreads + 0;
    int myStart = myPart * id;
...
int main(int argc, char* argv[]) {
  ...
  RasterCanvas c(-1, -1, w, h, "Dithered Points");
