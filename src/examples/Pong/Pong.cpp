...
Pong::Pong(Canvas& can, int & ballSpeed, int & paddleSpeed) {
    ...
    leftScore = new Text(-64, can.getWindowHeight()/2 - 40, 0, L"0", FONT, 32, 0,0,0, ColorFloat(0.0f, 0.0f, 1.0f, 1.0f));
    rightScore = new Text(64, can.getWindowHeight()/2 - 40, 0, L"0", FONT, 32, 0,0,0, ColorFloat(1.0f, 0.0f, 0.0f, 1.0f));
...
void Pong::draw(Canvas& can) {
    ...
    while (can.isOpen()) {
        ...
        // handle left
        if (pongBall->getX() - 8 < -can.getWindowWidth() / 2 + 32 &&
            pongBall->getX() - 8 > -can.getWindowWidth() / 2 + 16 &&
        ...
        // handle right
        else if (pongBall->getX() + 8 > can.getWindowWidth() / 2 - 32 &&
            pongBall->getX() + 8 < can.getWindowWidth() / 2 - 16 &&
