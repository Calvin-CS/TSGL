/*
 * test3DPhilosophers.cpp runs the Dining Philosphers Problem animation using the TSGL library and OpenMP.
 * This file includes a main method, Philospher3D class, Fork3D class, and Table3D class.
 *
 * The program provides a 3D visualization of the Dining Philosophers Problem
 ...
 *
 * Usage: ./test3DPhilosophers <numPhilosophers> <speed> <resolutionMethodChar>
...
#include <tsgl.h>
#include <random>
#include "DiningPhilosophers3D/Table3D.h"
#include "DiningPhilosophers3D/Philosopher3D.h"
...
void philosopherFunction(Canvas& can,int philosophers, std::string RM, bool step) {
  ...
  Table3D t(can,philosophers,method);
...
int main(int argc, char* argv[]) {
  if( argc == 1) {
    std::cout << "\nTo run the program with different values, use the format:\n\t./test3DPhilosophers <numPhilosophers> <speed> <resolutionMethodChar>"
    << "\nwhere <resolutionMethodChar> is a character specifying conflict resolution of the philosophers. Find options in tests/test3DPhilosophers.cpp" << std::endl;
  }
  ...
  Canvas c(-1, -1, 1300, 1000, "3D Dining Philosophers",1.0f/speed);
