...
void newtonPendulumFunction(Canvas& can) {
  ...
  while(can.getIsOpen()) {
    ...
    //stop value for circle: same as above
      while(rightMoving) {
        can.drawLine(centerX + 100, 0, centerX + maxStop, centerY - 21);
    ...
    can.drawLine(centerX - 100, 0, centerX - 100, centerY - 21);
    can.drawCircle(centerX - 100, centerY, RADIUS, 32, BLACK, false);
