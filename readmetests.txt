...
+ Some tests have I/O capabilities, in particular: testBallroom, testConway, testLangton testMandelbrot, testMouse, testPong, and testProjectiles.
...
+ Lastly, some tests will have two windows. One will be a progress bar/tiled square, the other will be a window for the test animation. Close each window with ESC key as normal.
    The progress bar shows how much work each thread needs to do, the tiled square shows how much work each thread did in the animation. In the test that displays the tiled square, 
   	a Legend will appear showing the color for each thread. 
-----------------------------------
DESCRIPTIONS
-----------------------------------
...
3). testBlurImage - Showcases an example usage of recursive splitting by blurring a given image. Takes in command-line arguments for the number of threads to use as well as for the
						image file. 
4). testCalcPi - Displays the integral for a Unit Circle and for the Cosine function using rectangles and trapezoids. Takes in command-line arguments for the number of segments and for the
...
11). testForestFire - Simulates a forest fire. Takes command-line arguments for the width and height of the screen.
12). testFunction - Visualizes the output of certain functions on the screen. In particular, the Cosine and Power functions as well as a special one. Takes command-line arguments for 
				      the width and height of the screen.
13). testGetPixels - Takes a few pixels from a photo and overwrites them with a new pixel that has a different color. Takes a command-line argument for the number of threads to use.
14). testGradientWheel - Shows a color wheel with a gradient color palate on a black background. Takes command-line arguments for the width and height of the screen. 
15). testGraydient - Shows a black-to-white gradient. Takes command-line arguments for the width and height of the screen as well as for the number of threads to use. 
16). testGreyscale - Takes pixels from an colored image and converts them to greyscale colors. Takes command-line arguments for the width and height of the screen as well as for the 
				        number of threads to use. 
17). testHighData - Simple stress test for the Canvas class where a large number of pixels are drawn to the screen at a high framerate. Takes command-line arguments for the width and height
					of the screen as well as for the number of threads to use. 
18). testImage - Draws a set of images on a screen. Takes command-line arguments for the width and height of the screen.
19). testImageCart - Draws a set of images on a screen that has a Cartesian coordinate system. Takes command-line arguments for the width and height of the screen. 
20). testInverter - Inverts the colors of an colored image to their greyscale colors. Takes a command-line argument for the number of the threads to use.
21). testLangton - Displays several visualizations of Langton's Ant (For a detailed look at Langton's Ant, go here: https://en.wikipedia.org/?title=Langton%27s_ant ). Takes command-line
...
22). testLineChain - Displays lines that move in a vivid pattern. Takes command-line arguments for the width and height of the screen as well as for the number of threads to use.
23). testLineFan - Displays a vibrant fan of colored lines. Takes command-line arguments for the width and height of the screen as well as for the number of threads to use.
24). testMandelbrot - Displays several visualizations of a Mandelbrot set. ( see http://linas.org/art-gallery/escape/smooth.html for what a Mandelbrot set is. ) Takes command-line
						arguments for the width and height of the screen as well as for the number of threads to use and the depth. There are four visualizations total.
						The visualizations are described below:
...
25). testMouse - A painting tool with the mouse on a white screen. Affectionately nicknamed "Art Creator". I/O capabilities:  Clicking and holding down on the left mouse button while dragging the mouse around on the 
...
26). testNewtonPendulum - Displays a simulation of Newton's Pendulum (or Newton's Cradle as some call it). See https://en.wikipedia.org/wiki/Newton's_cradle for more details.
							Takes command-line arguments for the width and height of the screen as well as for the number of threads.
							*NOTE* The number of threads must be an odd number.
27). testNova - Displays a Nova fractal. See http://en.wikipedia.org/wiki/Nova_fractal for more details. Takes command-line arguments for the width and height of the screen as well as
			      for the number of threads to use.
28). testPong - Play the hit old-school game, Pong. I/O capabilities: The w and s keys are for moving the left paddle up and down (respectively) and the up and down arrow keys move the
...
29). testProgressBar - Displays the progress bar that is shown in testMandelbrot. Takes command-line arguments for the width and height of the screen.
30). testProjectiles - Play a target shooting game. There are 10 targets and you have to hit the center of the target in order to get a point. Your score is printed after all 10 targets have 
...
31). testSeaUrchin - Displays colorful sea urchins. Each one is drawn on a different thread. If you close the window, the message "YOU KILLED MY SEA URCHINS! : ' (" is outputted to the
...
32). testScreenshot - Displays multiple colorful triangles then takes a screenshot of each drawing frame. The screenshots are saved in a similar fashion as they are in the 
                      	...
                      	screen.
33). testSmartSort - Visualizes the bottom-up mergesort algorithm. Takes command-line arguments for the number of elements to sort as well as for the number of threads to use.
34). testSpectrum - Draws the full spectrum of colors across the screen and changes over time. Takes a command-line argument for the number of threads to use in drawing.
...
36). testText - Displays text on the screen. Takes command-line arguments for the width and height of the screen as well as for the font file to use.
37). testTextCart - Displays text on a screen that has a Cartesian coordinate system. Takes command-line arguments for the width and height of the screen as well as for the font file to use.
38). testTextTwo - Tests the library's ability to set a default font if one is not specified. Takes command-line arguments for the width and height of the screen.
39). testUnits - Unit tests for a set number of key classes used in the library. 
40). testVoronoi - Displays two visualizations of a Voronoi diagram. Takes command-line arguments for the width and height of the screen as well as for the number of threads to use.
						The visualizations are described below:
